...
class RoutingTable:
    ...
    def __init__(self, mac, iface, out_addr, out_mask, clients):
        ...
        self.net_addr = lambda ip, mask: inet_ntoa(bytes.fromhex(hex(ip_to_int(ip) & ip_to_int(inet_aton(mask)) + 1)[2:].zfill(8)))
        self.out_net = self.net_addr(out_addr, out_mask)

        self.forbidden = ()
        self.hosts = {}
        # Add the default gateway to the table:
        self.__getitem__(inet_aton(wmi.WMI().query('select DefaultIPGateway from Win32_NetworkAdapterConfiguration '
    ...
    def __getitem__(self, addr):
        ...
        if self.net_addr(addr, self.out_net) != self.out_net and self.hosts:
